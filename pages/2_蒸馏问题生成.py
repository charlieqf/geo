...
from src.ui_presenters import question_table_height
...
with st.popover(QUESTION_PAGE["prompt_button"]):
    st.markdown(f"#### {QUESTION_PAGE['prompt_system_title']}")
    st.code(
        (config.prompts_dir / "question_pool_system.md").read_text(encoding="utf-8"),
        language="markdown",
    )
    st.markdown(f"#### {QUESTION_PAGE['prompt_user_title']}")
    st.code(
        (config.prompts_dir / "question_pool_user.md").read_text(encoding="utf-8"),
        language="markdown",
    )
    st.markdown(f"#### {QUESTION_PAGE['prompt_meta_title']}")
    st.json(prompt_meta)

with st.form("generate-question-draft"):
    keyword_col, brand_col, count_col = st.columns(3)
    with keyword_col:
        keyword = st.text_input(QUESTION_PAGE["keyword_label"], value="中国 GEO 服务")
    with brand_col:
...
if submitted:
    analyzer = OpenAIAnalysisClient(config.openai_api_key or "", config.openai_model)
...
if not drafts:
    ...
    with st.container(height=question_table_height(len(selected.get("questions", [])))):
        for item in selected.get("questions", []):
            ...
            st.markdown(
                ...
                <div class="geo-question-card">
                  ...
                  <div class="geo-question-reason">生成原因：{item["why_this_question"]}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
